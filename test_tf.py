...
def test_loss(arr1, arr2):
    ...
    loss_feat = tf.sqrt(tf.reduce_sum(
        tf.pow(tf.subtract(model1_l2, model2_l2), 2), 1, keepdims=False))
...
def test_torch_loss(arr1, arr2):
    ...
    loss_feat = torch.sqrt(torch.sum(
        torch.pow(torch.subtract(model1_l2, model2_l2), 2), 1, keepdim=False))
    loss_feat = torch.mean(loss_feat, dim=0, keepdim=False)
    print(loss_feat)
