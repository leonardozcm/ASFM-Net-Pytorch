...
def test_backbone(cfg, epoch_idx=-1, test_data_loader=None, test_writer=None, encoder=None, decoder=None, steps=0):
    ...
    if encoder is None:
        ...
        print(encoder.load_state_dict(checkpoint['model']))
        ...
        for k, v in checkpoint['model'].items():
            if k.startswith("module.decoder"):
                k_ = '.'.join(k.split('.')[2:])
                update_dict["module."+k_] = v
        print(decoder.load_state_dict(update_dict, strict=False))
        ...
        for model_idx, (taxonomy_id, model_id, data) in enumerate(t):
            ...
            with torch.no_grad():
                ...
                if y_coarse.shape[1] != gt.shape[1]:
                    ...
                    # print(coarse_gt.size())  # 4096
                loss_coarse = chamfer_sqrt(y_coarse, coarse_gt)

                cd_coarse = loss_coarse.item() * 1e3

                _metrics = [loss_coarse]
                test_losses.update([cd_coarse])
