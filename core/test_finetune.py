...
def test_finetune(cfg, epoch_idx=-1, test_data_loader=None, test_writer=None, model=None):
    ...
    with tqdm(test_data_loader) as t:
        for model_idx, (taxonomy_id, model_id, data) in enumerate(t):
            ...
            with torch.no_grad():
                ...
                _, pcds_pred = model(partial)
                losses = get_loss_finetune(pcds_pred, partial, gt, sqrt=True)
    ...
    if test_writer is not None:
        ...
        test_writer.add_scalar('Loss/Epoch/partial_matching',
                               test_losses.avg(4), epoch_idx)
