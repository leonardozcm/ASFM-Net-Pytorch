...
from snowmodels.utils import MLP_Res, MLP_CONV, fps_subsample, Transformer
from snowmodels.skip_transformer import SkipTransformer

# from snowmodels.utils import PointNet_SA_Module_KNN
...
class Decoder(nn.Module):
    ...
    def forward(self, feat, partial, return_P0=False):
        ...
        """
        arr_pcd = []
        pcd = self.decoder_coarse(feat).permute(
        ...
        # [p0(coarse 256), p1(sample scale 512), p2(*4 2048), p3(*8 16384)]
        return arr_pcd
...
class SnowflakeNet(nn.Module):
    def __init__(self, dim_feat=512, num_pc=256, num_p0=512, radius=1, up_factors=None):
        ...
        self.decoder = Decoder(dim_feat=dim_feat, num_pc=num_pc,
                               num_p0=num_p0, radius=radius, up_factors=up_factors)
