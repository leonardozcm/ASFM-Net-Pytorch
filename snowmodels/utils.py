...
def sample_and_group(xyz, points, npoint, nsample, radius, use_xyz=True):
    ...
    new_xyz = gather_operation(xyz, furthest_point_sample(
        xyz_flipped, npoint))  # (B, 3, npoint)

    idx = ball_query(radius, nsample, xyz_flipped, new_xyz.permute(
        0, 2, 1).contiguous())  # (B, npoint, nsample)
...
def sample_and_group_all(xyz, points, use_xyz=True):
    ...
    idx = torch.arange(nsample, device=device).reshape(
        1, 1, nsample).repeat(b, 1, 1)
    if points is not None:
        if use_xyz:
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all:
            ...
            new_xyz, new_points, idx, grouped_xyz = sample_and_group(
                xyz, points, self.npoint, self.nsample, self.radius, self.use_xyz)
...
class PointNet_FP_Module(nn.Module):
    def __init__(self, in_channel, mlp, use_points1=False, in_channel_points1=None, if_bn=True):
        ...
        for out_channel in mlp:
            self.mlp_conv.append(
                Conv1d(last_channel, out_channel, if_bn=if_bn))
            last_channel = out_channel
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        """
        dist, idx = three_nn(xyz1.permute(0, 2, 1).contiguous(),
        ...
        interpolated_points = three_interpolate(
            points2, idx, weight)  # B, in_channel, N
...
def query_knn(nsample, xyz, new_xyz, include_self=True):
    ...
    # fetch nsamples' nearest points' indice
    idx = torch.argsort(
        sqrdists, dim=-1, descending=False)[:, :, pad: nsample+pad]
    return idx.int()
...
def sample_and_group_knn(xyz, points, npoint, k, use_xyz=True, idx=None):
    ...
    # (B, 3, npoint, nsample) npoint groups, nsamples points in each group.
    grouped_xyz = grouping_operation(xyz, idx)
    # offset from kernels (new_xyz is kernels)
...
class PointNet_SA_Module_KNN(nn.Module):
    def __init__(self, npoint, nsample, in_channel, mlp, if_bn=True, group_all=False, use_xyz=True, if_idx=False, **kwags):
        """
        Args:
            npoint: int, number of points to sample
            nsample: int, number of points in each local region
