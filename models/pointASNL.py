...
from snowmodels.utils import query_knn
...
def grouping(npoint, K, xyz, new_xyz, points):
    ...
    idx = query_knn(K, xyz, new_xyz).type(torch.int64).contiguous()
    ...
    if points is not None:
        ...
        new_points = torch.cat([grouped_xyz, grouped_feature], dim=-1)
    else:
...
class SampleWeight(nn.Module):
    ...
    def forward(self, new_point, grouped_xyz):
        ...
        normalized_xyz = grouped_xyz - \
            grouped_xyz[:, :, 0, :].view(B, N, 1, 3).repeat(1, 1, S, 1)
        # channel last -> channel first
        new_point = torch.cat(
            [normalized_xyz, new_point], -1).permute(0, 3, 1, 2)

        transformed_feature = self.tf_bn(self.transformed_feature(
            new_point)).permute(0, 2, 3, 1)  # channel first -> channel last
        ...
        weights = torch.matmul(transformed_new_point,
                               transformed_feature1.transpose(2, 3))
        ...
        new_group_features = torch.reshape(
            new_group_features, (B, N, S, C)).permute(0, 3, 1, 2)

        for i, conv in enumerate(self.mlp_convs):
            bn = self.mlp_bns[i]
            new_group_features = F.relu(bn(conv(new_group_features)), inplace=True) if i < len(
                self.mlp_convs) - 1 else F.softmax(bn(conv(new_group_features)), 2)
...
class Adaptivesampling(nn.Module):
    def __init__(self, nsample, group_feature_channel, num_neighbor=8):  # num channel
        ...
        self.sampleweight = SampleWeight(
            group_feature_channel, [32, 1+group_feature_channel])
    ...
    def forward(self, group_xyz, group_feature):
        ...
        sample_weight = self.sampleweight(
            shift_group_points, shift_group_xyz).clone().detach()
        sw1, sw2, sw3, _ = sample_weight.shape
        new_weight_xyz = sample_weight[:, :, :, 0].view(
            sw1, sw2, sw3, 1).repeat(1, 1, 1, 3)
...
class PointNonLocalCell(nn.Module):
    ...
    def forward(self, feature, new_point):
        ...
        new_nonlocal_point = self.nnp_bn(self.new_nonlocal_point(
            new_nonlocal_point)).permute(0, 2, 3, 1)
...
class PointASNLSetAbstraction(nn.Module):  # use_knn = true,
    ...
    def __init__(self, npoint, nsample, in_channel, mlp, as_neighbor=8):
        ...
        self.Adaptive_smapling = Adaptivesampling(
            nsample, in_channel, as_neighbor)
        self.PointNonLocalCell = PointNonLocalCell(
            in_channel-3, in_channel, [max(32, in_channel//2), mlp[-1]])

        self.skip_spatial = nn.Conv1d(in_channel + 3, mlp[-1], 1)
        self.ss_bn = nn.BatchNorm1d(mlp[-1])

        # matrix multiplied weight is (B, N, sample, 32)
        self.weighted_conv = nn.Conv2d(32, mlp[-1], (1, mlp[-1]))
    ...
    def forward(self, xyz, feature):
        ...
        grouped_xyz, new_points = grouping(
            self.npoint, self.nsample, xyz, new_xyz, feature)
        # print(new_xyz.size())
        ...
        '''Adaptive Sampling'''
        if num_points != self.npoint:
            new_xyz, new_feature = self.Adaptive_smapling(
                grouped_xyz, new_points)
        nB, nN, nC = new_xyz.shape

        # translation normalization
        grouped_xyz -= new_xyz.view(nB, nN, 1,
                                    nC).repeat(1, 1, self.nsample, 1)
        new_points = torch.cat([grouped_xyz, new_points], -1)

        '''Point nonlocal Cell'''  # NL is true
        nf0, nf1, nf2 = new_feature.shape
        new_nonlocal_point = self.PointNonLocalCell(
            feature, new_feature.view(nf0, 1, nf1, nf2))

        '''skip connection'''
        skip_spartial, _ = torch.max(new_points, 2)
        skip_spartial = skip_spartial.permute(0, 2, 1)
        skip_spartial = self.ss_bn(
            self.skip_spatial(skip_spartial)).permute(0, 2, 1)
        ...
        new_points = self.wc_bn(self.weighted_conv(
            new_points)).permute(0, 2, 3, 1)

        # (batch_size, npoints, mlp[-1])
        new_points = torch.squeeze(new_points, 2)
...
if __name__ == "__main__":
    sa1 = PointASNLSetAbstraction(
        npoint=512, nsample=32, in_channel=6, mlp=[64, 64, 128])
    xyz = torch.rand(6, 3, 4096)
    norm = torch.rand(6, 3, 4096)
    l1_xyz, l1_points = sa1(xyz, norm)
    print(l1_xyz.size())
